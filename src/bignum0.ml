module Stable = struct
  module V1 = struct
    ...
    module Of_string_internal : sig
      ...
      *)
      ...
      let of_scientific_string_components ~coefficient ~power =
        ...
        let power' =
          if Int.( > ) power 0 then make power' Z.one else make Z.one power'
        in
        ...
        then (
          ...
          | '.' ->
            ...
            then fail s
              ...
              decompose s ~length (succ i) ~state:(state lor has_dot) ~dot:i ~exp ~slash
          | '/' ->
          ...
          | 'e' | 'E' ->
            ...
            then fail s
              ...
              decompose s ~length (succ i) ~state:(state lor has_exp) ~dot ~exp:i ~slash
          | '+' | '-' ->
      ...
      let create t : t =
        ...
        then Atom (Z.to_string t.num)
          ...
          | Decimal { max_decimal_digits } ->
            Atom (to_float_string ~max_decimal_digits t)
          | Rational_not_decimal ->
    ...
    module Bin_rep_conversion = struct
      ...
      let to_binable = Zarith.Q.to_string
      let of_binable = Zarith.Q.of_string
    end

    include Binable.Stable.Of_binable.V1 (String) (Bin_rep_conversion)
  ...
  module V2 = struct
    ...
    include (
      ...
      with module For_testing := V1.For_testing
       and module Bin_rep_conversion := V1.Bin_rep_conversion)
    ...
    let bin_read_t buf ~pos_ref =
      ...
      | Tag.Zero -> zero
      | Tag.Int -> of_bigint (bin_read_z_as_int63 buf ~pos_ref)
      | Tag.Over_int ->
      ...
      | Tag.Over_1_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 1_000)
      | Tag.Over_10_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 10_000)
      | Tag.Over_100_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 100_000)
      | Tag.Over_1_000_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 1_000_000)
      | Tag.Over_10_000_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 10_000_000)
      | Tag.Over_100_000_000 ->
        let n = bin_read_z_as_int63 buf ~pos_ref in
        make n (Z.of_int 100_000_000)
...
include (
  ...
  with module For_testing := T.For_testing)
...
module For_testing = struct
  ...
  let of_float_dyadic = of_float_dyadic
  let to_float_string = to_float_string
