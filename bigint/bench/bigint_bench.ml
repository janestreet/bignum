...
module%bench [@name "vs. Big_int"] _ = struct
...
module%bench [@name "binio"] _ = struct
  ...
  let%bench_fun "poke V1" =
    ...
    fun () -> run_poke b Bigint.Stable.V1.bin_writer_t
  ;;

  let%bench_fun "poke V2" =
    let b = Iobuf.create ~len:100 in
    fun () -> run_poke b Bigint.Stable.V2.bin_writer_t
  ;;

  let create_peek bin_writer_t =
    let b_small = Iobuf.of_string (Bin_prot.Writer.to_string bin_writer_t small_number) in
    let b_medium =
      Iobuf.of_string (Bin_prot.Writer.to_string bin_writer_t medium_number)
    in
    let b_large = Iobuf.of_string (Bin_prot.Writer.to_string bin_writer_t large_number) in
